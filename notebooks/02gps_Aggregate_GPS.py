...
# %%
if False: # take subset of participants
    ...
    print("process all participants")
...
df_loc = df_loc_all[
    df_loc_all["local_start_time"] < (first_obs_per_customer + pd.Timedelta(days=14))
].copy()
...
home_extractor = gps_features.HomeClusterExtractor(
    ...
    speed_limit=1.4,
    max_distance=150,
    epsilon=100 / 6371000, # 100 / kms_per_radian
    min_samples=10,
    min_nights_obs=4,
    min_f_home=0.5,
...
plot_df = plot_df.dropna(subset=["GPS_coverage_pct", "GPS_mean_gap_seconds", "GPS_max_gap_seconds", "local_day", "customer"])
...
sns.histplot(plot_df["GPS_mean_gap_hours"], bins=25, kde=True, ax=axes[3])
