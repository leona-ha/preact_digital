...
def run_ema_mappings(df_ema, df_data, df_home_clusters=None):
    """
    ...
    # Run all mapping functions
    df_ema = map_heart_rate_to_ema(df_ema, df_data)
    df_ema = map_steps_to_ema(df_ema, df_data)
    df_ema = map_gps_and_transition_to_ema(df_ema, df_data, df_home_clusters)
    df_ema = map_activity_types_to_ema(df_ema, df_data)

    return df_ema


def clean_heart_rate_data(df_hr):
    """
    ...
    print("Heart Rate Data Cleaning Summary:")
...
def clean_step_data(df_steps):
    ...
    df_steps["startTimestamp"] = pd.to_datetime(
        df_steps["startTimestamp"], errors="coerce"
    )
    ...
    df_steps["duration_min"] = (
        df_steps["endTimestamp"] - df_steps["startTimestamp"]
    ).dt.total_seconds() / 60
    ...
    # Drop helper columns
    df_steps.drop(
        ["duration_min", "steps_per_min"], axis=1, inplace=True, errors="ignore"
    )

    # Print detailed cleaning summary
    print("Step Data Cleaning Summary:")
    print(f"Initial entries: {initial_count}")
    ...
    print(
        f"Removed due to exceeding steps per minute threshold: {steps_per_min_removed}"
    )
    print(f"Remaining entries: {final_count}")
...
def map_steps_to_ema(df_ema, df_data, batch_size=20):
    """
    ...
    Returns:
        pd.DataFrame: Updated EMA DataFrame with steps data.
    """
    df_ema = df_ema.copy()

    # Prepare data
    df_steps = df_data[df_data["type"] == "Steps"].copy()
    df_steps = clean_step_data(df_steps)
    ...
    df_ema["sensor_block_start"] = pd.to_datetime(df_ema["sensor_block_start"])
    df_ema["sensor_block_end"] = pd.to_datetime(df_ema["sensor_block_end"])
    df_ema["customer"] = df_ema["customer"].astype(str)
    df_steps["customer"] = df_steps["customer"].astype(str)

    # Filter steps data to match the overall EMA time range
    min_time = df_ema["sensor_block_start"].min()
    max_time = df_ema["sensor_block_end"].max()
    df_steps = df_steps[
    ...
    unique_users = df_ema["customer"].unique()
    ...
    for i in range(num_batches):
        ...
        ema_batch = df_ema[df_ema["customer"].isin(user_batch)]
        steps_batch = df_steps[df_steps["customer"].isin(user_batch)]

        # Merge EMA blocks with steps data
        ema_batch["key"] = 0
        steps_batch["key"] = 0
        df_joined = pd.merge(steps_batch, ema_batch, on=["key", "customer"]).drop(
            columns=["key"]
        )
        ...
        if not df_joined.empty:
            ...
            df_joined["overlap_start"] = df_joined[
                ["startTimestamp", "sensor_block_start"]
            ].max(axis=1)
            df_joined["overlap_end"] = df_joined[
                ["endTimestamp", "sensor_block_end"]
            ].min(axis=1)
            df_joined["overlap_duration"] = (
                df_joined["overlap_end"] - df_joined["overlap_start"]
            ).dt.total_seconds()
            df_joined["step_duration"] = (
                df_joined["endTimestamp"] - df_joined["startTimestamp"]
            ).dt.total_seconds()
            df_joined["proportion"] = (
                df_joined["overlap_duration"] / df_joined["step_duration"]
            )
            df_joined["weighted_steps"] = (
                df_joined["proportion"] * df_joined["doubleValue"]
            )

            # Aggregate steps by unique_blocks
            steps_summary = (
                df_joined.groupby("unique_blocks")["weighted_steps"].sum().reset_index()
            )
    ...
    if results:
        ...
        df_ema = pd.merge(df_ema, final_features, on="unique_blocks", how="left")
        df_ema["n_steps"] = df_ema["n_steps"].fillna(-1).astype(int)
    else:
...
def map_activity_types_to_ema(df_ema, df_data, batch_size=20):
    """
    ...
    Returns:
        pd.DataFrame: Updated EMA DataFrame with activity type data.
    """
    df_ema = df_ema.copy()

    # Check if necessary columns are present
    if "type" not in df_data.columns or "longValue" not in df_data.columns:
        print(
            "Error: Required columns 'ActivityType' or 'longValue' are missing from the data."
        )
        return df_ema

    # Filter for ActivityType data
    df_activities = df_data[df_data["type"] == "ActivityType"].copy()
    if df_activities.empty:
        print("Warning: No ActivityType data available.")
        for col in range(102, 108):
            df_ema[f"activity_{col}_minutes"] = -1
        return df_ema
    ...
    for i in range(num_batches):
        ...
        df_joined = pd.merge(activities_batch, ema_batch, on=["key", "customer"]).drop(
            columns=["key"]
        )
        ...
        if not df_joined.empty:
            ...
            activity_summary = (
                df_joined.groupby(["unique_blocks", "longValue"])["overlap_minutes"]
                .sum()
            ...
            activity_summary.columns = [
                f"activity_{int(col)}_minutes" for col in activity_summary.columns
            ]
    ...
    if results:
        ...
        for col in range(102, 108):
            ...
            if col_name not in df_ema.columns:
                df_ema[col_name] = 0

        # Assign -1 for blocks with no activity data
        df_ema.loc[
            ~df_ema["unique_blocks"].isin(final_features["unique_blocks"]),
            [f"activity_{col}_minutes" for col in range(102, 108)],
        ] = -1
        ...
        print(
            "No valid ActivityType overlaps found. Assigned -1 to all activity columns."
        )

    return df_ema


def map_heart_rate_to_ema(df_ema, df_data, compute_stats=True, batch_size=20):
    """
    ...
    # Clean / filter heart rate data
    df_hr_cleaned = clean_heart_rate_data(df_data[df_data["type"] == "HeartRate"])
            ...
            if compute_stats:
                ...
                def calculate_features(group):
                    ...
                    for _, row in group.iterrows():
                        overlap_start = max(
                            row["startTimestamp"], group["sensor_block_start"].iloc[0]
                        )
                        overlap_end = min(
                            row["endTimestamp"], group["sensor_block_end"].iloc[0]
                        )
                ...
                hr_features = (
                    df_joined.groupby("unique_blocks")
                ...
                hr_features = (
                    df_joined.groupby("unique_blocks")["longValue"].mean().reset_index()
                )
    ...
    df_ema = df_ema.fillna(-1)

    return df_ema


def map_gps_and_transition_to_ema(df_ema, df_data, df_home_clusters, batch_size=10):
    """
    ...
    Parameters:
        df_ema (pd.DataFrame): EMA data.
        df_data (pd.DataFrame): Sensor data.
        df_home_clusters (pd.DataFrame): Home clusters data.
    ...
    if df_home_clusters is None:
        raise ValueError("df_home_clusters is not provided.")

    # Prepare data
    df_home_clusters["customer"] = df_home_clusters["customer"].astype(str)
    df_ema["customer"] = df_ema["customer"].astype(str)
    df_home_clusters["startTimestamp"] = pd.to_datetime(
        df_home_clusters["startTimestamp"]
    )
    df_ema["sensor_block_start"] = pd.to_datetime(df_ema["sensor_block_start"])
    df_ema["sensor_block_end"] = pd.to_datetime(df_ema["sensor_block_end"])
    ...
    min_time = df_ema["sensor_block_start"].min()
    max_time = df_ema["sensor_block_end"].max()
    df_gps_filtered = df_home_clusters[
        (df_home_clusters["startTimestamp"] >= min_time)
        & (df_home_clusters["startTimestamp"] <= max_time)
    ]

    # Process in batches of users
    users = df_ema["customer"].unique()
    ...
    for i in range(num_batches):
        ...
        if not df_joined.empty:
            ...
            def calculate_features(group):
                ...
                n_GPS = group.shape[0]
                total_distance_km = (
                    group["distance"].sum() / 1000
                )  # Convert meters to kilometers

                # Handle at_home_minutes using shift
                group = group.sort_values("startTimestamp")
                group["next_startTimestamp"] = group["startTimestamp"].shift(
                    -1, fill_value=group["sensor_block_end"].iloc[-1]
                )
                group["duration"] = (
                    group["next_startTimestamp"] - group["startTimestamp"]
                ).dt.total_seconds()

                at_home_minutes = (
                    group[group["at_home"] == 1]["duration"].sum() / 60
                )  # Convert to minutes
